...
/*
NOTE: With API KEY, you are able to get all estimates for each indicator
, since the per page maximum is very high (or none at all?). 
Thus, this section can be done more efficiently
, without repeating API calls 16 (or so) times due to the per page limit.  
*/ 
foreach indicator in $indicatorlist{
	...
	using "http://api.dhsprogram.com/rest/dhs/data?indicatorIds=`indicator'&breakdown=all&perpage=5000&page=`num'", 
	table(Data) 
...
* directory clearning. drop all temp files. 	
	local num=1
	while `num'<17{
